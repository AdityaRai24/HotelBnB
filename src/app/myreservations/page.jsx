...
export default async function page() {
  ...
  return (
    <>
      ...
      {allUserData?.Reservations?.length === 0 || allUserData===null ? (
        <NoItems
          heading="Sorry you do not have any Reservations currently..."
          text="Explore more homes and make Reservations to see them here..."
        />
      ) : (
        <div className="container mx-auto px-5 lg:px-10">
          <div className="grid lg:grid-cols-4 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-8">
            {allUserData?.Reservations?.map((item) => {
              return (
                <ListingCard
                  pathname={"/"}
